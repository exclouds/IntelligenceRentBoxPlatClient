...
const user = {
  state: {
    user: '',
    status: '',
    id: getCookie('AbpUserId'),
    code: '',
    gsWorkNo: '',      //公司代码
    jtWorkNo: getCookie('AbpCompanyType'),      //用户性质
    token: getCookie('AbpAuthToken'), 
    tenantId:getCookie('AbpTenantId'),     //租户
    permission:JSON.parse(localStorage.getItem('permission')),  //权限码
    name: getCookie('ABPName'),
    deptId: '', //公司是否管理员
    deptName: '',
    avatar: '',
    introduction: '',
        ...
        loginByEmail(userInfo).then(res => {
          ...
          if(res.success) {
           ...
           // setCookie(" Abp.TenantId", res.result.userId, 360);
            ...
            // commit('SET_JTWORKNO', res.result.companyType.toString());
            // commit('SET_GSWORKNO', res.result.organizationCode);
            // commit('SET_TENANTID',res.result.tenantId);
            // commit('SET_DEPTID',res.result.isAdmin.toString());
    ...
    GetInfo({
      ...
      return new Promise((resolve, reject) => {
        ...
        getInfo(data).then(res => {     
          ...
          commit('SET_NAME', res.result.name);
          commit('SET_JTWORKNO', res.result.companyType);
          commit('SET_GSWORKNO', res.result.organizationCode);
          commit('SET_TENANTID',res.result.tenantId);
          commit('SET_DEPTID',res.result.isAdmin);
